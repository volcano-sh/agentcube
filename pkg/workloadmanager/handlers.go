...
// resolveOwnerID extracts the owner ID from the request, writing an error response and
// returning ok=false if extraction fails for a reason other than a missing identity header
// (which just means an anonymous/unowned sandbox).
func (s *Server) resolveOwnerID(c *gin.Context) (ownerID string, ok bool) {
...
func (s *Server) handleSandboxCreate(c *gin.Context, kind string) {
	...
	case types.AgentRuntimeKind:
		sandbox, sandboxEntry, networkPolicy, err = buildSandboxByAgentRuntime(sandboxReq.Namespace, sandboxReq.Name, ownerID, s.informers, s.config.RouterSelector, s.config.RouterNamespace, sandboxReq.NetworkPolicy)
	case types.CodeInterpreterKind:
		sandbox, sandboxClaim, networkPolicy, sandboxEntry, err = buildSandboxByCodeInterpreter(sandboxReq.Namespace, sandboxReq.Name, ownerID, s.informers, s.config.RouterSelector, s.config.RouterNamespace, sandboxReq.NetworkPolicy)
	}
...
// waitForSandboxResult blocks until the sandbox reports ready, the context is canceled, or the 2-minute deadline fires.
...
func (s *Server) handleDeleteSandbox(c *gin.Context) {
	...
	// The NP shares the sandbox name, so not-found is expected when no policy was created.
