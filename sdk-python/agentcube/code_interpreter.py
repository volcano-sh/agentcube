...
class CodeInterpreterClient:
    ...
    def _init_data_plane(self):
        ...
        self.dp_client = CodeInterpreterDataPlaneClient(
            ...
            auth=self._auth,
            on_session_not_found=self._invalidate_session,
        )
        if self.verbose:
            self.dp_client.logger.setLevel(logging.DEBUG)

    def _invalidate_session(self):
        """Forget a session that the Router reports as missing."""
        self.session_id = None

    def _require_data_plane(self) -> CodeInterpreterDataPlaneClient:
        if not self.session_id or not self.dp_client:
            raise SessionError("Session is no longer available; create a new client to continue")
        return self.dp_client
    ...
    def execute_command(self, command: str, timeout: Optional[float] = None) -> str:
        ...
        Parameters:
            ...
            timeout (Optional[float]): Maximum time in seconds to allow for command execution.
                If None (default), the SDK's default of 120 seconds is used.
        Returns:
            str: The output of the command.
        """
        return self._require_data_plane().execute_command(command, timeout)

    def execute_command_result(
        self, command: str, timeout: Optional[float] = None
    ) -> dict[str, Any]:
        """Run a shell command and return ``stdout``, ``stderr``, and ``exit_code`` (no raise on failure)."""
        return self._require_data_plane().execute_command_result(command, timeout)
    ...
    def run_code(self, language: str, code: str, timeout: Optional[float] = None) -> str:
        ...
        Args:
            ...
            timeout: Optional. The maximum time (in seconds) to wait for the code
                     execution to complete. If not provided, the SDK's default of
                     120 seconds is used.

        Returns:
            The standard output (stdout) generated by the code execution.
        """
        return self._require_data_plane().run_code(language, code, timeout)
    ...
    def write_file(self, content: str, remote_path: str):
        ...
        """
        self._require_data_plane().write_file(content, remote_path)
    ...
    def upload_file(self, local_path: str, remote_path: str):
        ...
        Args:
            ...
            remote_path: The destination path of the file in the remote environment.
                         This path is relative to the session's working directory.
        """
        self._require_data_plane().upload_file(local_path, remote_path)
    ...
    def download_file(self, remote_path: str, local_path: str):
        ...
        """
        self._require_data_plane().download_file(remote_path, local_path)
    ...
    def list_files(self, path: str = "."):
        ...
        """
        return self._require_data_plane().list_files(path)
