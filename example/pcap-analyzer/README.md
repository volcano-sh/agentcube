# 🧠 PCAP Analyzer — Architecture & Logic Overview
...
1. **Planner Agent** — Generates a one-shot Bash analysis script.  
2. **SandboxRunner** — Uploads the script and PCAP into a secure sandbox and executes it.  
3. **Reporter Agent** — Analyzes execution logs and produces a structured Markdown report.  
...
- **Output:**
  - `script` — Final Bash script generated by the LLM  
  - `results` — Execution logs and command outputs  
  - `report` — Human-readable Markdown summary report  
...
## 🔁 4. Automatic Repair Mechanism

### 🧩 Step 1 — Single Execution (`_execute_once_in_runner()`)
...
### 🧩 Step 2 — Repair Logic (`_repair_script()`)
...
### 🧩 Step 3 — Main Loop (`_analyze_with_retries()`)
...
## 💡 5. Prompt Template Design Philosophy
...
2. Call `_plan_script()` → Generate initial script.  
3. Call `_analyze_with_retries()` → Execute and auto-repair as needed.  
4. Call `_report()` → Produce Markdown report.  
5. Return consolidated results (`script`, `results`, `report`).

---

## ⚙️ 7. Configurable Parameters
...
🪪 License: *Specify here* (e.g., MIT, Apache 2.0)
