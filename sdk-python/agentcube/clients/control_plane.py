...
class ControlPlaneClient:
    ...
    def __init__(
        ...
        self.base_url = workload_manager_url or os.getenv("WORKLOADMANAGER_URL")
        if not self.base_url:
            raise ValueError("Workload Manager URL must be provided via 'workload_manager_url' argument or 'WORKLOADMANAGER_URL' environment variable.")
    ...
    def create_session(
        ...
        try:
            ...
            if "sessionId" not in data or not data["sessionId"]:
                self.logger.error(f"Response JSON missing 'sessionId': {data}")
