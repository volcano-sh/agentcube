...
func TestBuildSandboxByAgentRuntime_NotFound(t *testing.T) {
	...
	_, _, _, err := buildSandboxByAgentRuntime(testNamespace, "missing", "", ifm, nil, "", nil)
...
func TestBuildSandboxByAgentRuntime_Success(t *testing.T) {
	...
	sandbox, entry, _, err := buildSandboxByAgentRuntime(testNamespace, testAgentRuntimeName, "", ifm, nil, "", nil)
	if err != nil {
...
func TestBuildSandboxByCodeInterpreter_NotFound(t *testing.T) {
	...
	_, _, _, _, err := buildSandboxByCodeInterpreter(testNamespace, "missing", "", ifm, nil, "", nil)
...
func TestBuildSandboxByCodeInterpreter_PicodAuthFailsWithoutKey(t *testing.T) {
	...
	_, _, _, _, err = buildSandboxByCodeInterpreter(testNamespace, "ci-picod-no-key", "", ifm, nil, "", nil)
...
func TestBuildSandboxByCodeInterpreter_SuccessNoWarmPool(t *testing.T) {
	...
	sandbox, claim, _, entry, err := buildSandboxByCodeInterpreter(testNamespace, "ci-no-wp", "", ifm, nil, "", nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBuildSandboxByCodeInterpreter_SuccessWithWarmPool(t *testing.T) {
	...
	sandbox, claim, _, entry, err := buildSandboxByCodeInterpreter(testNamespace, testCodeInterpreterWarmPool, "", ifm, nil, "", nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
