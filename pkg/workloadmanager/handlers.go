...
func (s *Server) handleSandboxCreate(c *gin.Context, kind string) {
	...
	case types.AgentRuntimeKind:
		sandbox, sandboxEntry, err = buildSandboxByAgentRuntime(namespace, sandboxReq.Name, ownerID, s.informers)
	case types.CodeInterpreterKind:
		sandbox, sandboxClaim, sandboxEntry, err = buildSandboxByCodeInterpreter(namespace, sandboxReq.Name, ownerID, s.informers)
	default:
	...
	if err != nil {
		klog.Errorf("build sandbox failed for %s/%s: %v", namespace, sandboxReq.Name, err)
