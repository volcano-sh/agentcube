...
- **Common build targets (from repo root)**: `make build` (workloadmanager), `make build-router`, `make build-all`. Regenerate code/CRDs/clients with `make generate`, `make gen-client`, or `make gen-all`; verify with `make gen-check`. Go toolchain version is specified in `go.mod`.
