...
Examples:
  ...
  Output: {"task": "Go to https://kubernetes.io/releases/ and extract the latest release version number and key highlights from the release notes.", "allowed_domains": ["kubernetes.io"]}

  User: "Search for the best Python web frameworks in 2025"
  Output: {"task": "Search Google for 'best Python web frameworks 2025', open the top 3 results, and summarize the recommended frameworks with pros and cons.", "allowed_domains": []}
"""
...
class PlaywrightMCPClient:
    ...
    async def run_task(
        ...
        async def capture_response(response: httpx.Response) -> None:
            ...
            if new_session_id:
                ...
                if http_client is not None:
                    http_client.headers[AGENTCUBE_SESSION_HEADER] = new_session_id
        ...
        async with httpx.AsyncClient(
            ...
            async with streamable_http_client(self.base_url, http_client=http_client) as (
                read_stream,
                write_stream,
                _,
            ):
