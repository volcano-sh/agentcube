...
// TestReconciler_Reconcile_DeleteErrorHandling tests error handling during deletion
func TestReconciler_Reconcile_DeleteErrorHandling(t *testing.T) {
	testScheme := setupTestScheme()
	now := time.Now()

	// Test that NotFound errors during deletion are ignored
	t.Run("Delete of already deleted sandbox should ignore NotFound error", func(t *testing.T) {
		sandbox := &sandboxv1alpha1.Sandbox{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "expired-sandbox",
				Namespace: "default",
		...
		// First reconcile should delete the sandbox
		result, err := reconciler.Reconcile(context.Background(), reconcile.Request{
			NamespacedName: types.NamespacedName{
				Name:      sandbox.Name,
		...
		// Second reconcile should handle NotFound gracefully
		result, err = reconciler.Reconcile(context.Background(), reconcile.Request{
			NamespacedName: types.NamespacedName{
				Name:      sandbox.Name,
				Namespace: sandbox.Namespace,
...
// TestReconciler_SetupWithManager tests controller setup
// Note: SetupWithManager requires a real controller-runtime Manager which needs k8s config.
// This is typically tested in integration tests. Here we verify the method signature and basic structure.
func TestReconciler_SetupWithManager(t *testing.T) {
	testScheme := setupTestScheme()

	// Verify that SetupWithManager method exists and has correct signature
	reconciler := &Reconciler{
		Scheme: testScheme,
	}

	// The method should exist and be callable
	// In a real scenario, this would be tested with envtest or integration tests
	assert.NotNil(t, reconciler.SetupWithManager, "SetupWithManager method should exist")

	// Verify the reconciler structure
...
func TestReconciler_Reconcile_EdgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify sandbox still exists (should not be deleted for these cases)
			sandbox := &sandboxv1alpha1.Sandbox{}
			err = fakeClient.Get(context.Background(), types.NamespacedName{
				Name:      tt.sandbox.Name,
				Namespace: tt.sandbox.Namespace,
			...
			// Future time should result in requeue
			if tt.name == "Sandbox with future last-activity-time should not be deleted" {
				assert.True(t, result.RequeueAfter > 0, "Future time should result in requeue")
			}
		})
	}
}
