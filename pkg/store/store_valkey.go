...
func (vs *valkeyStore) loadSandboxesBySessionIDs(ctx context.Context, sessionIDs []string) ([]*types.SandboxStore, error) {
	if len(sessionIDs) == 0 {
		return nil, nil
	}
	...
	sandboxResults := make([]*types.SandboxStore, 0, len(stingSliceResults))
	for i, sandboxObjString := range stingSliceResults {
		...
		var sandboxRedis types.SandboxStore
...
func (vs *valkeyStore) GetSandboxBySessionID(ctx context.Context, sessionID string) (*types.SandboxStore, error) {
...
func (vs *valkeyStore) StoreSandbox(ctx context.Context, sandboxStore *types.SandboxStore) error {
	if sandboxStore == nil {
...
func (vs *valkeyStore) UpdateSandbox(ctx context.Context, sandboxStore *types.SandboxStore) error {
	if sandboxStore == nil {
...
func (vs *valkeyStore) ListExpiredSandboxes(ctx context.Context, before time.Time, limit int64) ([]*types.SandboxStore, error) {
	if limit <= 0 {
		return nil, nil
	}
...
func (vs *valkeyStore) ListInactiveSandboxes(ctx context.Context, before time.Time, limit int64) ([]*types.SandboxStore, error) {
	if limit <= 0 {
		return nil, nil
	}
