...
* **AgentCube Cluster**: A running AgentCube cluster with Workload Manager and Router deployed.
* **CodeInterpreter CR**: A `CodeInterpreter` resource must exist in your cluster before using the SDK. See [example/code-interpreter/code-interpreter.yaml](../../example/code-interpreter/code-interpreter.yaml) for a sample configuration.
