...
func TestRecordStickyNode(t *testing.T) {
	...
	t.Run("no-op when shutting down", func(_ *testing.T) {
		server := &Server{k8sClient: &K8sClient{}}
		server.shuttingDown = true
		entry := &sandboxEntry{StickyWorkloadName: "ar-1", StickyWorkloadKind: runtimev1alpha1.AgentRuntimeKind}
		// Must not spawn a goroutine (no wg.Add) and must not panic.
		server.recordStickyNode("ns", entry, "node-1")
	})

	t.Run("concurrent recordStickyNode does not block", func(_ *testing.T) {
		// RWMutex with RLock allows concurrent readers, so multiple
		// recordStickyNode calls should not block each other.
		// Provide a fake cubeClientset so the async goroutine does not panic.
		cs := cubefake.NewSimpleClientset()
		server := &Server{k8sClient: &K8sClient{cubeClientset: cs}}
		entry := &sandboxEntry{StickyWorkloadName: "ar-1", StickyWorkloadKind: runtimev1alpha1.AgentRuntimeKind}
