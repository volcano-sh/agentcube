...
type Store interface {
	...
	GetSandboxBySessionID(ctx context.Context, sessionID string) (*types.SandboxStore, error)
	// StoreSandbox store sandbox into storage
	StoreSandbox(ctx context.Context, sandboxStore *types.SandboxStore) error
	// UpdateSandbox update sandbox of storage
	UpdateSandbox(ctx context.Context, sandboxStore *types.SandboxStore) error
	...
	ListExpiredSandboxes(ctx context.Context, before time.Time, limit int64) ([]*types.SandboxStore, error)
	// ListInactiveSandboxes returns up to limit sandboxes with last-activity time before the given time
	ListInactiveSandboxes(ctx context.Context, before time.Time, limit int64) ([]*types.SandboxStore, error)
