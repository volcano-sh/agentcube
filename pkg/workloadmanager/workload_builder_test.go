...
func TestInjectPreferredNodeAffinity(t *testing.T) {
	...
	t.Run("appends on top of equivalent hostname preference so scheduler sums weights", func(t *testing.T) {
		// The scheduler sums weights of matching PreferredDuringScheduling terms.
		// Weight values must stay in 1–100 (API validation), so merging into the
		// existing term would produce an invalid spec. Appending a separate
		// weight-100 term on top of user-defined weight-1 gives the sticky node
		// a total score of 101 while keeping both terms valid.
		userTerm := hostnameTerm("node-d")
		userTerm.Weight = 1
		podSpec := &corev1.PodSpec{
			Affinity: &corev1.Affinity{
				NodeAffinity: &corev1.NodeAffinity{
		...
		preferred := podSpec.Affinity.NodeAffinity.PreferredDuringSchedulingIgnoredDuringExecution
		assert.Len(t, preferred, 2, "stickiness must be appended as a separate term")
		assert.EqualValues(t, 1, preferred[0].Weight, "user-defined weight must be preserved")
		assert.EqualValues(t, 100, preferred[1].Weight, "stickiness term must have weight 100")
	})
